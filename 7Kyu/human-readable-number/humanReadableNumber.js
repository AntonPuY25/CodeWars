function toReadable (number) {
    ...
    if(number<=19 ){
        ...
        return arr2[number.toString()]
    }else if(number>20 && number < 100){
        const arrNumbers = number.toString().split('')
        const endingTy = `${ arrNumbers[0]}0`
        return `${arr2[endingTy]} ${arr[arrNumbers[1]]}`
    }else if(number>= 100){
        const arrNumbers = number.toString().split('')
        const copyArrNumbers = Number([...arrNumbers].splice(1,).join(''))
        const endingTy = `${ arrNumbers[1]}0`
        if(copyArrNumbers>=10 && copyArrNumbers <= 19){
            return `${arr[arrNumbers[0]]} hundred ${arr[copyArrNumbers.toString()]}`
        }
        if(arrNumbers[1] === '0' && arrNumbers[2] === '0' ){
            return `${arr[arrNumbers[0]]} hundred`
        }
       else if(arrNumbers[1] === '0'){
            return `${arr[arrNumbers[0]]} hundred ${arr[arrNumbers[2]]}`
        }else if(arrNumbers[2] === '0'){
            return `${arr[arrNumbers[0]]} hundred ${arr2[endingTy]}`
        }else{
           return   `${arr[arrNumbers[0]]} hundred ${arr2[endingTy]} ${arr[arrNumbers[2]]}`
        }
    }
}
